...
select distinct lbd.BOOK_TITLE,lbd.PUBLICATION,lbd.PUBLISH_DATE,lsd.SUPPLIER_NAME,lsd.contact
from LMS_SUPPLIERS_DETAILS lsd inner join LMS_BOOK_DETAILS lbd
on(lsd.supplier_id=lbd.supplier_id);   

select distinct mem.MEMBER_NAME,mem.MEMBERSHIP_STATUS
from LMS_MEMBERS mem inner join LMS_BOOK_ISSUE fd
on(mem.MEMBER_ID=fd.MEMBER_ID)
inner join LMS_FINE_DETAILS fi
on(fi.FINE_RANGE=fd.FINE_RANGE)
where fi.FINE_AMOUNT>74;


alter table LMS_fine_details rename column fine_rANGE TO MY_FINE_RANGE
alter table LMS_fine_details rename column MY_FINE_RANGE TO FINE_RANGE
...
mem.MEMBER_NAME,mem.MEMBERSHIP_STATUS

select distinct mem.MEMBER_NAME,mem.MEMBERSHIP_STATUS,sd.EMAIL
from LMS_MEMBERS mem inner join LMS_BOOK_ISSUE fd
on(mem.MEMBER_ID=fd.MEMBER_ID)
inner join LMS_FINE_DETAILS fi
on(fi.FINE_RANGE=fd.FINE_RANGE)
inner join LMS_BOOK_DETAILS bd
on (bd.BOOK_CODE = fd.BOOK_CODE)
...
on(sd.SUPPLIER_ID=bd.SUPPLIER_ID)
where trim(lower(sd.ADDRESS)) in('pune','mumbai','delhi')
and bd.SUPPLIER_ID is null;

select distinct bt.BOOK_TITLE,bt.BOOK_EDITION,bt.PUBLICATION
from LMS_BOOK_DETAILS bt left join LMS_BOOK_ISSUE bi
on(bt.BOOK_CODE=bi.BOOK_CODE)
