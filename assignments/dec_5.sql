...
select BOOK_TITLE,PUBLISH_DATE from LMS_BOOK_DETAILS
where PUBLISH_DATE>to_date('01-01-2005','dd-mm-yyyy');
...
-- Use varchar instead of char as it automatically adds spaces according to array length.

create table list(names varchar(10));

insert into list(names) values ('CABLE');
insert into list(names) values ('TABLE');
insert into list(names) values ('ABLE');
insert into list(names) values ('CAB');
insert into list(names) values ('ABIL');
insert into list(names) values ('CABINET');
insert into list(names) values ('POLYCAB');
insert into list(names) values ('KHBDKHBDS');
insert into list(names) values ('djagfjhf');
insert into list(names) values ('KJHSKAJSKNBSSKJS');

select * from list;

select * from list where upper(trim(names)) like 'CA_';

select * from list where upper(names) like 'CA_';

desc list;

select * from list where upper(names) like 'CA________';

select names,length(names) from list;
...
create table myTable (my_city varchar(10));


insert into myTable values('Pune');
insert into myTable values('mumbai');
insert into myTable values('dubai');
insert into myTable values('Karjat');
insert into myTable values('PCMC');
insert into myTable values('JAPAN');
insert into myTable values('USA');
insert into myTable values('Pune');

select * from myTable;

SELECT my_city,
DECODE(my_city,'mumbai','RENAMED_mumbai','Pune','RENAMED_pune','DEFAULT_VALUE')  DECODE_EXP from myTable;


SELECT my_city,
CASE myTable.my_city
    WHEN 'mumbai' THEN 'RENAMED_CHENNAI'
    WHEN 'Pune' THEN 'RENAMED_PUNE'
    ELSE
    'DEFAULT_VALUE' 
END SIMPLE_CASE_EXPR from myTable;


SELECT my_city,
CASE 
    WHEN my_city = 'dubai' THEN 'RENAMED_CHENNAI'
    WHEN my_city = 'Pune' THEN 'RENAMED_PUNE'
    ELSE
    'DEFAULT_VALUE' 
